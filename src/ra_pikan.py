...
class RAPiKANConfig:
    ...
    n_aux_diag: int = 5000       # Dense auxiliary set for residual diagnosis.
    ...
    min_improvement: float = 0.10    # Minimum relative L2 drop to continue full training.
    n_targeted_cycles: int = 2       # Cycles 0..n_targeted_cycles-1 use targeted extension;
                                     # remaining cycles use uniform extension to avoid
    ...
    for cycle in range(cfg.n_cycles):
        ...
        if avg_grid_size < cfg.max_grid_size:
            if cycle < cfg.n_targeted_cycles:
                ...
                # Uniform: spread knots across the full domain to avoid ill-conditioned
                # lstsq when input-layer knots are already very concentrated from prior
                # targeted extensions.
        ...
        # rather than pure RAD (which fully replaces the collocation set each cycle).
        #
