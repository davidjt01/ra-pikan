...
class ChebyKANLayer(nn.Module):
    ...
    def __init__(
        ...
        # Learnable coefficients: shape (out_features, in_features, degree+1)
    ...
    def _chebyshev(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Stack along last dim: (batch, in_features, degree+1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        T = self._chebyshev(x)  # (batch, in_features, degree+1)
        # y[b, j] = sum_i sum_d coef[j, i, d] * T[b, i, d]
        # Einsum: 'bid, jid -> bj'
...
def b_spline_basis(
    ...
    g_l = grid[:-1].unsqueeze(0)   # (1, n-1)
    g_r = grid[1:].unsqueeze(0)    # (1, n-1)
...
class BSplineKANLayer(nn.Module):
    ...
    def __init__(
        ...
        k = spline_order
        # Base grid: (grid_size+1,) for each dimension
        base = torch.linspace(grid_range[0], grid_range[1], grid_size + 1)
        # Extended grid: (grid_size + 2*(k-1) + 1,)
        ext = _extend_grid(base, k)  # (grid_size + 2k - 1,)

        # Store grids as a list (one per input dimension).
        # Registered as buffers so they follow .to(device) calls.
        # We register them with a name pattern.
        self._n_dims = in_features
        for i in range(in_features):
            self.register_buffer(f"grid_{i}", ext.clone())

        n_basis = grid_size + k - 1  # n_knots - k = (grid_size + 2k-1) - k
        self.coef = nn.Parameter(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.in_features):
            xi = x[:, i]   # (batch,)
            grid_i = self._grid(i)
            B = b_spline_basis(xi, grid_i, self.spline_order)  # (batch, n_basis_i)
            nb = B.shape[1]  # actual n_basis for this dimension
            # coef[:, i, :nb] shape (out_features, n_basis_i)
            # y += B @ coef[:, i, :nb].T -> (batch, out_features)
            y = y + B @ self.coef[:, i, :nb].T
        return y

    # ------------------------------------------------------------------
    # Grid extension helpers
    # ------------------------------------------------------------------
    ...
    def extend_grid_targeted(
        ...
        k = self.spline_order
        # Extract inner (non-repeated) knots
        inner = old_grid[k - 1: -(k - 1)]  # (G+1,) for grid_size G
        # New knot positions in sub-domain
        new_knots = torch.linspace(
            sub_low, sub_high, n_new_points + 2,
            device=old_grid.device, dtype=old_grid.dtype,
        )[1:-1]  # exclude endpoints (already present)
        # Merge and sort
        ...
        # Use the actual merged knots directly as the extended grid
    ...
    def _extend_dim(
        ...
        if fit_pts is not None and fit_pts.shape[0] > 0:
            xi = fit_pts[:, dim]  # (batch,)
            # Evaluate old basis at fit_pts
            with torch.no_grad():
                B_old = b_spline_basis(xi, old_grid, k)  # (batch, old_n_basis)
                # Use only the valid (non-padded) coefficients for this dimension
                old_coef_dim = self.coef[:, dim, :old_n_basis]  # (out_features, old_n_basis)
                # Current function values: old_coef_dim @ B_old.T -> (out, batch)
                old_vals = old_coef_dim @ B_old.T  # (out_features, batch)
                # Evaluate new basis
                B_new = b_spline_basis(xi, new_grid, k)  # (batch, new_n_basis)
                # Least-squares: B_new @ w.T ≈ old_vals.T  -> w.T ≈ pinv(B_new) @ old_vals.T
                # w shape: (out_features, new_n_basis)
                B_new_np = B_new.cpu().numpy()
                old_vals_np = old_vals.T.cpu().numpy()  # (batch, out_features)
                w_np = np.linalg.lstsq(B_new_np, old_vals_np, rcond=None)[0]  # (new_n_basis, out)
                new_coef_dim = torch.tensor(
                    w_np.T, dtype=self.coef.dtype, device=self.coef.device
                )  # (out_features, new_n_basis)
        else:
            # No fit points: pad/truncate with zeros
        ...
        # Pad shorter dims to max basis size
        ...
        for t in all_dims:
            ...
            padded.append(t)
        new_coef = nn.Parameter(torch.cat(padded, dim=1))  # (out, in, max_n)
        self.coef = new_coef
