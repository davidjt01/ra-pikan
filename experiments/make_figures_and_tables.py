...
def save_accuracy_table(df: pd.DataFrame) -> None:
    ...
    # Also pretty-print to stdout with columns in group order
    print("\n=== Accuracy Table ===")
...
# ---------------------------------------------------------------------------
# Figure: bar chart of final L2 errors
# ---------------------------------------------------------------------------
...
def plot_symbolic_summary() -> None:
    ...
    # Bar chart: distribution of R² values
    fig, ax = plt.subplots(figsize=(6, 4))
...
# ---------------------------------------------------------------------------
# Figure: solution field comparisons
# ---------------------------------------------------------------------------
...
def plot_solution_fields_1d(benchmark: str, param_key: str, param_val: str,
    ...
    for ax, method in zip(axes[:-1], methods):
        ...
        pts = data["pts"]   # (N, 2) for 1D PDE (t, x)
        pred = data["pred"]
        exact = data["exact"]

        # Plot at final time slice (t closest to 1.0) or spatial profile
        if pts.shape[1] == 2:
            t_vals = pts[:, 0]
            x_vals = pts[:, 1]
            t_max = t_vals.max()
...
def plot_residual_maps(benchmark: str, param_key: str, param_val: str,
    ...
    if not files:
        ...
        return
    data = np.load(files[0])
    aux_pts = data["aux_pts"]   # (n_cycles, N, 2)
    residuals = data["residuals"]  # (n_cycles, N)
...
# ---------------------------------------------------------------------------
# Figure: Gini evolution across all benchmarks
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Figure: KAN activation functions (Duffing)
# ---------------------------------------------------------------------------
...
def plot_kan_activations(output_name: str = "fig_kan_activations_duffing.pdf") -> None:
    ...
    n_cols = len(found)
    # Plot layer 0 (10 funcs) and output layer (10 funcs) side by side per method
...
# ---------------------------------------------------------------------------
# Figure: sensitivity analysis (percentile threshold)
# ---------------------------------------------------------------------------
...
def plot_sensitivity(output_name: str = "fig_sensitivity_percentile.pdf") -> None:
    ...
    for ax, pe in zip(axes, pes):
        ...
        # Plot stable points with log scale
        ax.set_yscale("log")
        ax.errorbar(
        ...
        # Mark diverged points with an X at the top of the axis
